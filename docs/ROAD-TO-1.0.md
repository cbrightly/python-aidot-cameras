...
| `rc19` | 2026-09-12 | The A001513 emits in-order packets stamped ~1.7 s in the past every 30 s, so the serve now stamps by arrival instead of trusting the camera clock | **Measured on the box: 6.1-6.8 backward-DTS warnings/streaming-minute -> 0, A/V skew 2.7-3.9 s -> 0 ms, decode errors unchanged.** Keep |
