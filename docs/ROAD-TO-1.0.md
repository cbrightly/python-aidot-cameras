...
| `rc13` | 2026-09-07 | The cameras type their detections - human, vehicle, package, pet - and the library exposed only motion on/off plus a sensitivity number. Adds `getRoiHuman`/`setRoiHuman` as `async_get_detection_types` / `async_set_detection_type`. Probed read-only on an A000088 and an A001064 first, then the setter verified on both and the cameras restored | No. A settings path; nothing on the streaming path changed. Read-only probes preceded every write, and a camera that does not answer reads as unknown rather than all-off |
| `rc14` | 2026-09-07 | The peer id announced a random client class, out of the vendor's range 11 times in 16; it now announces 2 (WEB), what the vendor web app sends. Screened at 9/9 A000088 across three runs including one on a drained fleet with HA disabled, against 0/9 for the class-0 arm reverted on 08-31 | No. Nothing on the media path changed; the class was screened under the same condition the earlier failure was found in before shipping |
| `rc19` | 2026-09-12 | The A001513 emits in-order packets stamped ~1.7 s in the past every 30 s, so the serve now stamps by arrival instead of trusting the camera clock | **Watch it.** It changes timestamps on every SDES serve. Shipped to be measured against a 6.1-6.8 warnings/streaming-minute baseline; revert if it does not beat it |
| `rc18` | 2026-09-08 | Battery cameras dropped control commands: the wake and the command were published together, so the camera was still waking when the command arrived. Wake now settles first | No. It only adds a delay before a command on a battery camera, and only when one is not already awake |
| `rc17` | 2026-09-08 | The integration had no way to ask what a MODEL declares, only what the cloud reports - and the two differ, which is how rc16's duration control reached a model that ignores it | No. Adds a read-only field parsed from the device dict |
| `rc16` | 2026-09-07 | The settings under "light up when someone appears" (`LingerDuration`, `Dimming`) had no API. Both verified on hardware by read-back. `lightBehavior` acks without landing and is documented as such rather than exposed | No. A settings path, and it never writes the automation master |
| `rc15` | 2026-09-07 | The camera's automatic siren (`getAutoAlarm`/`setAutoAlarm`) had no API - a master switch plus motion/person triggers, independent of the motion-detection and ROI settings already exposed | No. A settings path. The setter was verified on both mains models with the siren master left disarmed throughout, so nothing sounded |
