...
def test_media_flowing_not_stalled():
    ...
    assert (
        SdesSession.is_stalled(
            last_media=1000.0, started_at=900.0, now=1005.0, watchdog=_W, grace=_G
        )
        is False
    )
...
def test_media_stalled_after_watchdog():
    ...
    assert (
        SdesSession.is_stalled(
            last_media=1000.0, started_at=900.0, now=1031.0, watchdog=_W, grace=_G
        )
        is True
    )
...
def test_media_never_started_within_grace():
    ...
    assert (
        SdesSession.is_stalled(
            last_media=0.0, started_at=1000.0, now=1040.0, watchdog=_W, grace=_G
        )
        is False
    )
...
def test_media_never_started_past_grace():
    ...
    assert (
        SdesSession.is_stalled(
            last_media=0.0, started_at=1000.0, now=1061.0, watchdog=_W, grace=_G
        )
        is True
    )
