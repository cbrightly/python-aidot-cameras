...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void setMeasuredDimension(Rect childrenBounds, int wSpec, int hSpec) {
        ...
        if (this.mOrientation == 1) {
            width = RecyclerView.LayoutManager.chooseSize(hSpec, childrenBounds.height() + verticalPadding, getMinimumHeight());
            usedHeight = RecyclerView.LayoutManager.chooseSize(wSpec, (this.mSizePerSpan * this.mSpanCount) + horizontalPadding, getMinimumWidth());
        } else {
            usedHeight = RecyclerView.LayoutManager.chooseSize(wSpec, childrenBounds.width() + horizontalPadding, getMinimumWidth());
            width = RecyclerView.LayoutManager.chooseSize(hSpec, (this.mSizePerSpan * this.mSpanCount) + verticalPadding, getMinimumHeight());
        }
                ...
                for (int i2 = 0; i2 < childCount; i2++) {
                    ...
                    if (!lp.mFullSpan) {
                        if (!isLayoutRTL() || this.mOrientation != 1) {
                            int i3 = lp.mSpan.mIndex;
                            int newOffset = this.mSizePerSpan * i3;
                            ...
                            int i5 = lp.mSpan.mIndex;
                            child2.offsetLeftAndRight(((-((i4 - 1) - i5)) * this.mSizePerSpan) - ((-((i4 - 1) - i5)) * before));
                        }
                    }
                }
            }
        ...
        if (i > 0) {
            if (i == this.mSpanCount) {
                for (int i2 = 0; i2 < this.mSpanCount; i2++) {
                    ...
                    int line = savedState2.mSpanOffsets[i2];
    ...
    private void measureChildWithDecorationsAndMargin(View child, LayoutParams lp, boolean alreadyMeasured) {
        if (lp.mFullSpan) {
            if (this.mOrientation == 1) {
                measureChildWithDecorationsAndMargin(child, this.mFullSizeSpec, RecyclerView.LayoutManager.getChildMeasureSpec(getHeight(), getHeightMode(), getPaddingTop() + getPaddingBottom(), lp.height, true), alreadyMeasured);
            } else {
                measureChildWithDecorationsAndMargin(child, RecyclerView.LayoutManager.getChildMeasureSpec(getWidth(), getWidthMode(), getPaddingLeft() + getPaddingRight(), lp.width, true), this.mFullSizeSpec, alreadyMeasured);
            }
        } else if (this.mOrientation == 1) {
            measureChildWithDecorationsAndMargin(child, RecyclerView.LayoutManager.getChildMeasureSpec(this.mSizePerSpan, getWidthMode(), 0, lp.width, false), RecyclerView.LayoutManager.getChildMeasureSpec(getHeight(), getHeightMode(), getPaddingTop() + getPaddingBottom(), lp.height, true), alreadyMeasured);
        } else {
            measureChildWithDecorationsAndMargin(child, RecyclerView.LayoutManager.getChildMeasureSpec(getWidth(), getWidthMode(), getPaddingLeft() + getPaddingRight(), lp.width, true), RecyclerView.LayoutManager.getChildMeasureSpec(this.mSizePerSpan, getHeightMode(), 0, lp.height, false), alreadyMeasured);
        }
    }
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec, boolean alreadyMeasured) {
        ...
        int widthSpec2 = updateSpecWithExtra(widthSpec, i + rect.left, lp.rightMargin + rect.right);
        int i2 = lp.topMargin;
        Rect rect2 = this.mTmpRect;
        int heightSpec2 = updateSpecWithExtra(heightSpec, i2 + rect2.top, lp.bottomMargin + rect2.bottom);
        if (alreadyMeasured) {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (lazySpanLookup == null || (iArr = lazySpanLookup.mData) == null) {
            ...
            state.mSpanLookup = iArr;
            state.mSpanLookupSize = iArr.length;
            state.mFullSpanItems = lazySpanLookup.mFullSpanItems;
        }
        if (getChildCount() > 0) {
            if (this.mLastLayoutFromEnd) {
        ...
        if (getChildCount() != 0 && delta != 0) {
            ...
            for (int i = 0; i < this.mSpanCount; i++) {
                ...
                if (layoutState.mItemDirection == -1) {
                    ...
                    distance = i2 - this.mSpans[i].getStartLine(i2);
                } else {
                    distance = this.mSpans[i].getEndLine(layoutState.mEndLine) - this.mLayoutState.mEndLine;
                }
                if (distance >= 0) {
            ...
            for (int i3 = 0; i3 < itemPrefetchCount && this.mLayoutState.hasMore(state); i3++) {
        ...
        public View getFocusableViewAfter(int referenceChildPosition, int layoutDir) {
            ...
            if (layoutDir != -1) {
                for (int i = this.mViews.size() - 1; i >= 0; i--) {
                    ...
                    if (staggeredGridLayoutManager.mReverseLayout && staggeredGridLayoutManager.getPosition(view) >= referenceChildPosition) {
                        break;
                    }
                    StaggeredGridLayoutManager staggeredGridLayoutManager2 = StaggeredGridLayoutManager.this;
                    if ((!staggeredGridLayoutManager2.mReverseLayout && staggeredGridLayoutManager2.getPosition(view) <= referenceChildPosition) || !view.hasFocusable()) {
                        break;
                    }
                    candidate = view;
                }
                ...
                for (int i2 = 0; i2 < limit; i2++) {
                    ...
                    if (staggeredGridLayoutManager3.mReverseLayout && staggeredGridLayoutManager3.getPosition(view2) <= referenceChildPosition) {
                        break;
                    }
                    StaggeredGridLayoutManager staggeredGridLayoutManager4 = StaggeredGridLayoutManager.this;
                    if ((!staggeredGridLayoutManager4.mReverseLayout && staggeredGridLayoutManager4.getPosition(view2) >= referenceChildPosition) || !view2.hasFocusable()) {
                        break;
                    }
                    candidate = view2;
                }
    ...
    public static class LazySpanLookup {
        ...
        private void offsetFullSpansForRemoval(int positionStart, int itemCount) {
            ...
            if (list != null) {
                ...
                for (int i = list.size() - 1; i >= 0; i--) {
                    ...
                    int i2 = fsi.mPosition;
                    if (i2 >= positionStart) {
                        if (i2 < end) {
                            this.mFullSpanItems.remove(i);
                        } else {
                            fsi.mPosition = i2 - itemCount;
                        }
                    }
                }
            }
        ...
        public FullSpanItem getFirstFullSpanItemInRange(int minPos, int maxPos, int gapDir, boolean hasUnwantedGapAfter) {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                if (i2 >= minPos && (gapDir == 0 || fsi.mGapDir == gapDir || (hasUnwantedGapAfter && fsi.mHasUnwantedGapAfter))) {
                    return fsi;
                }
            }
            return null;
