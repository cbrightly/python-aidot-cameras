...
public final class GapWorker implements Runnable {
        ...
        for (int i2 = 0; i2 < viewCount; i2++) {
            ...
            if (view2.getWindowVisibility() == 0) {
                ...
                for (int j = 0; j < prefetchRegistry.mCount * 2; j += 2) {
                    ...
                    int[] iArr = prefetchRegistry.mPrefetchArray;
                    int distanceToItem = iArr[j + 1];
                    task.immediate = distanceToItem <= viewVelocity;
                    task.viewVelocity = viewVelocity;
                    task.distanceToItem = distanceToItem;
                    task.view = view2;
                    task.position = iArr[j];
                    totalTaskIndex++;
                }
            }
        }
    ...
    private void flushTaskWithDeadline(Task task, long deadlineNs) {
        RecyclerView.ViewHolder holder = prefetchPositionWithDeadline(task.view, task.position, task.immediate ? DefaultAuthCallbacks.CREDENTIALS_NEVER_EXPIRE : deadlineNs);
