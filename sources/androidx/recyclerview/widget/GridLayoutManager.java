...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void setMeasuredDimension(Rect childrenBounds, int wSpec, int hSpec) {
        ...
        if (this.mOrientation == 1) {
            ...
            usedHeight = RecyclerView.LayoutManager.chooseSize(wSpec, iArr[iArr.length - 1] + horizontalPadding, getMinimumWidth());
        } else {
            int width2 = RecyclerView.LayoutManager.chooseSize(wSpec, childrenBounds.width() + horizontalPadding, getMinimumWidth());
            int[] iArr2 = this.mCachedBorders;
            usedHeight = width2;
            width = RecyclerView.LayoutManager.chooseSize(hSpec, iArr2[iArr2.length - 1] + verticalPadding, getMinimumHeight());
        }
        ...
        if (layingOutInPrimaryDirection) {
            while (span > 0) {
                ...
                if (i > 0) {
                    ...
                    span = getSpanIndex(recycler, state, i2);
                } else {
                    return;
                }
            }
    ...
    public void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        if (flexibleInOtherDir2) {
            ...
            int maxSize4 = 0;
            for (int i3 = 0; i3 < itemSpanSize; i3++) {
        ...
        while (i4 < itemSpanSize) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view5) != maxSize) {
                ...
                if (this.mOrientation == 1) {
                    ...
                    wSpec = RecyclerView.LayoutManager.getChildMeasureSpec(totalSpaceInOther, 1073741824, horizontalInsets, lp.width, false);
                    ...
                    hSpec = RecyclerView.LayoutManager.getChildMeasureSpec(totalSpaceInOther, 1073741824, verticalInsets, lp.height, false);
                }
    ...
    private void measureChild(View view, int otherDirParentSpecMode, boolean alreadyMeasured) {
        ...
        if (this.mOrientation == 1) {
            wSpec = RecyclerView.LayoutManager.getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, horizontalInsets, lp.width, false);
            hSpec = RecyclerView.LayoutManager.getChildMeasureSpec(this.mOrientationHelper.getTotalSpace(), getHeightMode(), verticalInsets, lp.height, true);
        } else {
            hSpec = RecyclerView.LayoutManager.getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, verticalInsets, lp.height, false);
            wSpec = RecyclerView.LayoutManager.getChildMeasureSpec(this.mOrientationHelper.getTotalSpace(), getWidthMode(), horizontalInsets, lp.width, true);
        }
