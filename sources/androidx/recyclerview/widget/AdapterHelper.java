...
public final class AdapterHelper implements OpReorderer.Callback {
    ...
    private void dispatchAndUpdateViewHolders(UpdateOp op) {
        ...
        int tmpStart = updatePositionWithPostponed(op.positionStart, i);
        int tmpCnt = 1;
        ...
        for (int p = 1; p < op.itemCount; p++) {
            ...
            if (continuous) {
                ...
                UpdateOp tmp = obtainUpdateOp(i2, tmpStart, tmpCnt, op.payload);
    ...
    private int updatePositionWithPostponed(int pos, int cmd) {
        ...
        for (int i = this.mPostponedList.size() - 1; i >= 0; i--) {
            ...
            int i2 = postponed.cmd;
            if (i2 == 8) {
                int i3 = postponed.positionStart;
                int i4 = postponed.itemCount;
                if (i3 < i4) {
                ...
                if (pos < start || pos > end) {
                    if (pos < i3) {
                        if (cmd == 1) {
                            postponed.positionStart = i3 + 1;
                            postponed.itemCount = i4 + 1;
                        } else if (cmd == 2) {
                            postponed.positionStart = i3 - 1;
                            postponed.itemCount = i4 - 1;
                        }
                ...
                int i5 = postponed.positionStart;
                if (i5 <= pos) {
                    if (i2 == 1) {
        ...
        for (int i6 = this.mPostponedList.size() - 1; i6 >= 0; i6--) {
            UpdateOp op = this.mPostponedList.get(i6);
            if (op.cmd == 8) {
                int i7 = op.itemCount;
                if (i7 == op.positionStart || i7 < 0) {
                    this.mPostponedList.remove(i6);
                    recycleUpdateOp(op);
                }
    ...
    public static final class UpdateOp {
        ...
        public String toString() {
            return Integer.toHexString(System.identityHashCode(this)) + Constants.ARRAY_TYPE + cmdToString() + ",s:" + this.positionStart + "c:" + this.itemCount + ",p:" + this.payload + "]";
        }
