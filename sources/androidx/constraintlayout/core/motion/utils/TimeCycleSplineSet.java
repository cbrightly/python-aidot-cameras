...
public abstract class TimeCycleSplineSet {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.count; i++) {
            str = str + Constants.ARRAY_TYPE + this.mTimePoints[i] + " , " + df.format(this.mValues[i]) + "] ";
        }
        return str;
    }
    ...
    public static class CustomSet extends TimeCycleSplineSet {
        ...
        public boolean setProperty(MotionWidget view, float t, long time, KeyCache cache) {
            ...
            if (Float.isNaN(this.last_cycle)) {
                float floatValue = cache.getFloatValue(motionWidget, this.mAttributeName, 0);
            ...
            float f = (float) ((((double) this.last_cycle) + ((((double) delta_time) * 1.0E-9d) * ((double) period))) % 1.0d);
            this.last_cycle = f;
            this.last_time = j;
            float wave = calcWave(f);
            ...
            while (true) {
                ...
                this.mContinue = (((double) fArr3[i]) != 0.0d) | z;
                fArr2[i] = (fArr3[i] * wave) + offset;
                i++;
                long j2 = time;
            }
