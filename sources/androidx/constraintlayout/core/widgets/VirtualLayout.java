    ...
    public boolean measureChildren() {
        ...
        while (true) {
            ...
            if (widget != null && !(widget instanceof Guideline)) {
                ...
                if (widthBehavior != dimensionBehaviour || widget.mMatchConstraintDefaultWidth == 1 || heightBehavior != dimensionBehaviour || widget.mMatchConstraintDefaultHeight == 1) {
                    skip = false;
                }
                if (!skip) {
                    if (widthBehavior == dimensionBehaviour) {
                        widthBehavior = ConstraintWidget.DimensionBehaviour.WRAP_CONTENT;
                    }
                    if (heightBehavior == dimensionBehaviour) {
                        heightBehavior = ConstraintWidget.DimensionBehaviour.WRAP_CONTENT;
                    }
                    BasicMeasure.Measure measure = this.mMeasure;
                    measure.horizontalBehavior = widthBehavior;
                    measure.verticalBehavior = heightBehavior;
