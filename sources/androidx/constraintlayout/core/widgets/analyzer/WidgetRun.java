...
public abstract class WidgetRun implements Dependency {
    ...
    public long wrapSize(int direction) {
        ...
        long size = (long) dimensionDependency.value;
    ...
    public final DependencyNode getTarget(ConstraintAnchor anchor) {
        ConstraintAnchor constraintAnchor = anchor.mTarget;
        if (constraintAnchor == null) {
            return null;
        }
        ConstraintWidget targetWidget = constraintAnchor.mOwner;
        switch (AnonymousClass1.$SwitchMap$androidx$constraintlayout$core$widgets$ConstraintAnchor$Type[constraintAnchor.mType.ordinal()]) {
            case 1:
    ...
    public void updateRunCenter(Dependency dependency, ConstraintAnchor startAnchor, ConstraintAnchor endAnchor, int orientation2) {
        ...
        if (startTarget.resolved && endTarget.resolved) {
            ...
            DimensionDependency dimensionDependency = this.dimension;
            if (dimensionDependency.resolved) {
                if (dimensionDependency.value == distance) {
    ...
    private void resolveDimension(int orientation2, int distance) {
        ...
        switch (this.matchConstraintsType) {
            ...
            case 2:
                ...
                if (parent != null) {
                    DimensionDependency dimensionDependency = (orientation2 == 0 ? parent.horizontalRun : parent.verticalRun).dimension;
                    if (dimensionDependency.resolved) {
                        ConstraintWidget constraintWidget = this.widget;
                        this.dimension.resolve(getLimitedDimension((int) ((((float) dimensionDependency.value) * (orientation2 == 0 ? constraintWidget.mMatchConstraintPercentWidth : constraintWidget.mMatchConstraintPercentHeight)) + 0.5f), orientation2));
                        return;
                    }
                    return;
                }
            ...
            case 3:
                ...
                WidgetRun run = constraintWidget2.horizontalRun;
                ConstraintWidget.DimensionBehaviour dimensionBehaviour = run.dimensionBehavior;
                ConstraintWidget.DimensionBehaviour dimensionBehaviour2 = ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT;
                if (dimensionBehaviour == dimensionBehaviour2 && run.matchConstraintsType == 3) {
                ...
                if (orientation2 == 0) {
                    run = constraintWidget2.verticalRun;
                }
                if (run.dimension.resolved) {
                    float ratio = constraintWidget2.getDimensionRatio();
                    if (orientation2 == 1) {
                        value = (int) ((((float) run.dimension.value) / ratio) + 0.5f);
                    } else {
                        value = (int) ((((float) run.dimension.value) * ratio) + 0.5f);
                    }
    ...
    public final int getLimitedDimension(int dimension2, int orientation2) {
        if (orientation2 == 0) {
            ...
            int value = Math.max(constraintWidget.mMatchConstraintMinWidth, dimension2);
            if (max > 0) {
        ...
        ConstraintWidget constraintWidget2 = this.widget;
        int max2 = constraintWidget2.mMatchConstraintMaxHeight;
        int value2 = Math.max(constraintWidget2.mMatchConstraintMinHeight, dimension2);
        if (max2 > 0) {
