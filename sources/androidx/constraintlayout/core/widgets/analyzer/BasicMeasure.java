...
public class BasicMeasure {
    ...
    public void updateHierarchy(ConstraintWidgetContainer layout) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (horizontalDimensionBehaviour == dimensionBehaviour || widget.getVerticalDimensionBehaviour() == dimensionBehaviour) {
    ...
    private void measureChildren(ConstraintWidgetContainer layout) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (!(child instanceof Guideline) && !(child instanceof Barrier) && !child.isInVirtualLayout() && (!optimize || (horizontalWidgetRun = child.horizontalRun) == null || (verticalWidgetRun = child.verticalRun) == null || !horizontalWidgetRun.dimension.resolved || !verticalWidgetRun.dimension.resolved)) {
                boolean skip = false;
                ...
                ConstraintWidget.DimensionBehaviour dimensionBehaviour = ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT;
                if (widthBehavior == dimensionBehaviour && child.mMatchConstraintDefaultWidth != 1 && heightBehavior == dimensionBehaviour && child.mMatchConstraintDefaultHeight != 1) {
                    skip = true;
                }
                if (!skip && layout.optimizeFor(1) && !(child instanceof VirtualLayout)) {
                    if (widthBehavior == dimensionBehaviour && child.mMatchConstraintDefaultWidth == 0 && heightBehavior != dimensionBehaviour && !child.isInHorizontalChain()) {
                        skip = true;
                    }
                    if (heightBehavior == dimensionBehaviour && child.mMatchConstraintDefaultHeight == 0 && widthBehavior != dimensionBehaviour && !child.isInHorizontalChain()) {
                        skip = true;
                    }
                    if ((widthBehavior == dimensionBehaviour || heightBehavior == dimensionBehaviour) && child.mDimensionRatio > 0.0f) {
                        skip = true;
                    }
                }
                if (!skip) {
    ...
    public long solverMeasure(ConstraintWidgetContainer layout, int optimizationLevel, int paddingX, int paddingY, int widthMode, int widthSize, int heightMode, int heightSize, int lastMeasureWidth, int lastMeasureHeight) {
        ...
        if (optimize2) {
            ...
            while (i4 < childCount2) {
                ConstraintWidget child = constraintWidgetContainer2.mChildren.get(i4);
                ConstraintWidget.DimensionBehaviour horizontalDimensionBehaviour = child.getHorizontalDimensionBehaviour();
                ConstraintWidget.DimensionBehaviour dimensionBehaviour = ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT;
                boolean matchWidth = horizontalDimensionBehaviour == dimensionBehaviour;
                boolean optimize3 = optimize2;
                boolean matchHeight2 = child.getVerticalDimensionBehaviour() == dimensionBehaviour;
        ...
        if (matchHeight && (metrics = LinearSystem.sMetrics) != null) {
            metrics.measures++;
        }
        ...
        if (optimize4) {
            ...
            if (allSolved2) {
                ...
                constraintWidgetContainer2.updateFromRuns(z4, i3 == 1073741824);
            }
            int i5 = widthSize2;
        ...
        if (!allSolved || computations != 2) {
            ...
            if (childCount2 > 0) {
                ...
                boolean z7 = optimizeWrap;
                z3 = false;
            }
            int sizeDependentWidgetsCount4 = sizeDependentWidgetsCount;
            if (sizeDependentWidgetsCount4 > 0) {
                boolean needSolverPass2 = false;
                ConstraintWidget.DimensionBehaviour horizontalDimensionBehaviour2 = layout.getHorizontalDimensionBehaviour();
                ConstraintWidget.DimensionBehaviour dimensionBehaviour2 = ConstraintWidget.DimensionBehaviour.WRAP_CONTENT;
                boolean containerWrapWidth = horizontalDimensionBehaviour2 == dimensionBehaviour2 ? z2 : z3;
                boolean containerWrapHeight = layout.getVerticalDimensionBehaviour() == dimensionBehaviour2 ? z2 : z3;
