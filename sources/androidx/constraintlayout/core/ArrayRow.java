        ...
        for (int i = 0; i < currentSize; i++) {
            ...
            if (currentValue < 0.0f) {
                ...
                if ((avoid == null || !avoid[v.id]) && v != exclude) {
                    if (1 != 0) {
                        SolverVariable.Type type = v.mType;
                        if ((type == SolverVariable.Type.SLACK || type == SolverVariable.Type.ERROR) && currentValue < value) {
                            value = currentValue;
                            pivot = v;
                        }
                    } else {
                        SolverVariable.Type type2 = v.mType;
                        if (type2 == SolverVariable.Type.SLACK) {
                            if (currentValue < valueSlack) {
                                valueSlack = currentValue;
                                pivotSlack = v;
                            }
                        } else if (type2 == SolverVariable.Type.ERROR && currentValue < value) {
                            value = currentValue;
                            pivot = v;
                        }
                    }
