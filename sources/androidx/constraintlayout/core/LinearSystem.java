...
public class LinearSystem {
    ...
    public void reset() {
        ...
        while (true) {
            ...
            SolverVariable variable = solverVariableArr[i];
            if (variable != null) {
                variable.reset();
            }
            i++;
    ...
    public SolverVariable createObjectVariable(Object anchor) {
        ...
        if (anchor instanceof ConstraintAnchor) {
            ...
            if (i == -1 || i > this.mVariablesID || this.mCache.mIndexedVariables[i] == null) {
                ...
                this.mCache.mIndexedVariables[i2] = variable;
            }
        }
        return variable;
    }
    ...
    public SolverVariable createSlackVariable() {
        ...
        this.mNumColumns++;
        variable.id = i;
        this.mCache.mIndexedVariables[i] = variable;
        return variable;
    }
    ...
    /* access modifiers changed from: package-private */
    public void minimizeGoal(Row goal) {
        Metrics metrics = sMetrics;
        if (metrics != null) {
    ...
    public final void cleanupRows() {
        ...
        while (i2 < this.mNumRows) {
            ...
            if (current.isSimpleDefinition) {
                SolverVariable solverVariable = current.variable;
                solverVariable.computedValue = current.constantValue;
                solverVariable.removeFromRow(current);
                int j = i2;
                while (true) {
                    i = this.mNumRows;
                    if (j >= i - 1) {
        ...
        if (SIMPLIFY_SYNONYMS && this.hasSimpleDefinition) {
            ...
            while (i3 < this.mNumRows) {
                ...
                if (arrayRowArr2[i3] != null && arrayRowArr2[i3].isSimpleDefinition) {
                    ArrayRow removedRow = arrayRowArr2[i3];
                    ...
                    while (true) {
                        ...
                        arrayRowArr3[j - 1] = arrayRowArr3[j];
                        if (arrayRowArr3[j - 1].variable.definitionId == j) {
                            arrayRowArr3[j - 1].variable.definitionId = j - 1;
                        }
                        lastRow = j;
                        j++;
                    }
    ...
    public void removeRow(ArrayRow row) {
        ...
        int i;
        if (row.isSimpleDefinition && (solverVariable = row.variable) != null) {
            if (solverVariable.definitionId != -1) {
                ...
                while (true) {
                    ...
                    ArrayRow[] arrayRowArr = this.mRows;
                    SolverVariable rowVariable = arrayRowArr[i2 + 1].variable;
            ...
            SolverVariable solverVariable2 = row.variable;
            if (!solverVariable2.isFinalValue) {
                solverVariable2.setFinalValue(this, row.constantValue);
            }
            if (OPTIMIZED_ENGINE) {
                this.mCache.optimizedArrayRowPool.release(row);
            } else {
        ...
        while (!done) {
            ...
            if (pivotCandidate != null) {
                ...
                for (int i3 = 0; i3 < this.mNumRows; i3++) {
                    ArrayRow current = this.mRows[i3];
                ...
                if (pivotRowIndex > -1) {
                    ...
                    SolverVariable solverVariable = pivotEquation.variable;
                    solverVariable.definitionId = pivotRowIndex;
                    solverVariable.updateReferencesWithNewDefinition(this, pivotEquation);
                }
            } else {
                done = true;
            }
    ...
    public void displayReadableRows() {
        ...
        for (int i2 = 0; i2 < this.mVariablesID + 1; i2++) {
            ...
            SolverVariable variable2 = solverVariableArr[i2];
            if (variable2 != null && variable2.isSynonym) {
                s2 = s2 + " ~[" + i2 + "] => " + variable2 + " = " + solverVariableArr[variable2.synonym] + " + " + variable2.synonymDelta + "\n";
            }
        }
        String s3 = s2 + "\n\n #  ";
        for (int i3 = 0; i3 < this.mNumRows; i3++) {
    ...
    public void addCenterPoint(ConstraintWidget widget, ConstraintWidget target, float angle, int radius) {
        ...
        SolverVariable Al = createObjectVariable(constraintWidget.getAnchor(type));
        ConstraintAnchor.Type type2 = ConstraintAnchor.Type.TOP;
        SolverVariable At = createObjectVariable(constraintWidget.getAnchor(type2));
        ConstraintAnchor.Type type3 = ConstraintAnchor.Type.RIGHT;
        SolverVariable Ar = createObjectVariable(constraintWidget.getAnchor(type3));
        ConstraintAnchor.Type type4 = ConstraintAnchor.Type.BOTTOM;
        SolverVariable Ab = createObjectVariable(constraintWidget.getAnchor(type4));
        SolverVariable Bl = createObjectVariable(constraintWidget2.getAnchor(type));
        SolverVariable Bt = createObjectVariable(constraintWidget2.getAnchor(type2));
        SolverVariable Br = createObjectVariable(constraintWidget2.getAnchor(type3));
        SolverVariable Bb = createObjectVariable(constraintWidget2.getAnchor(type4));
        ArrayRow row = createRow();
        row.createRowWithAngle(At, Ab, Bt, Bb, (float) (Math.sin((double) f) * ((double) i)));
        addConstraint(row);
        ArrayRow row2 = createRow();
        row2.createRowWithAngle(Al, Ar, Bl, Br, (float) (Math.cos((double) f) * ((double) i)));
        addConstraint(row2);
    }
}
