...
public class MotionLabel extends View implements FloatLayout {
        ...
        if (attrs != null) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.MotionLabel_android_text) {
                    ...
                    float dimension = a.getDimension(attr, this.mRound);
                    this.mRound = dimension;
                    if (Build.VERSION.SDK_INT >= 21) {
                        setRound(dimension);
                    }
                } else if (attr == R.styleable.MotionLabel_borderRoundPercent) {
                    float f = a.getFloat(attr, this.mRoundPercent);
    ...
    private float getVerticalOffset() {
        ...
        float boxHeight = ((Float.isNaN(this.mFloatHeight) ? (float) getMeasuredHeight() : this.mFloatHeight) - ((float) getPaddingTop())) - ((float) getPaddingBottom());
        float f = fm.descent;
        float f2 = fm.ascent;
        return (((boxHeight - ((f - f2) * scale)) * (1.0f - this.mTextPanY)) / 2.0f) - (f2 * scale);
    }
    ...
    public void buildShape(float scale) {
        if (this.mUseOutline || scale != 1.0f) {
            ...
            rect2.bottom = (float) getHeight();
            rect2.right = (float) getWidth();
    ...
    public void setTextSize(float size) {
        this.mTextSize = size;
        String str = TAG;
        Log.v(str, Debug.getLoc() + JustifyTextView.TWO_CHINESE_BLANK + size + " / " + this.mBaseTextSize);
