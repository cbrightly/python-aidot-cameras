...
public class ConstraintLayoutStates {
    ...
    public boolean needsToChange(int id, float width, float height) {
        ...
        if ((i2 == -1 || !state.mVariants.get(i2).match(width, height)) && this.mCurrentConstraintNumber != state.findMatch(width, height)) {
            return true;
        }
        return false;
    }
    ...
    public void updateConstraints(int id, float width, float height) {
        ...
        if (i == id) {
            ...
            int i2 = this.mCurrentConstraintNumber;
            if ((i2 == -1 || !state.mVariants.get(i2).match(width, height)) && this.mCurrentConstraintNumber != (match = state.findMatch(width, height))) {
                if (match == -1) {
