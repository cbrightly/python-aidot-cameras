...
public class ViewPager extends ViewGroup {
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        if (oldCurInfo != null) {
            ...
            int itemIndex = curItem.position;
            if (oldCurPosition < itemIndex) {
                int itemIndex2 = 0;
                float offset = oldCurInfo.offset + oldCurInfo.widthFactor + marginOffset;
                int pos = oldCurPosition + 1;
                while (pos <= curItem.position && itemIndex2 < this.mItems.size()) {
                    Object obj = this.mItems.get(itemIndex2);
                    while (true) {
                        ii2 = (ItemInfo) obj;
                        if (pos > ii2.position && itemIndex2 < this.mItems.size() - 1) {
                            itemIndex2++;
        ...
        int i3 = curItem.position;
        int pos3 = i3 - 1;
        this.mFirstOffset = i3 == 0 ? curItem.offset : -3.4028235E38f;
        this.mLastOffset = i3 == N + -1 ? (curItem.offset + curItem.widthFactor) - 1.0f : Float.MAX_VALUE;
        int i4 = curIndex - 1;
        while (i4 >= 0) {
            ItemInfo ii3 = this.mItems.get(i4);
            while (true) {
                i2 = ii3.position;
                if (pos3 <= i2) {
                    break;
        ...
        int i5 = curIndex + 1;
        while (i5 < itemCount) {
            ItemInfo ii4 = this.mItems.get(i5);
            while (true) {
                i = ii4.position;
                if (pos4 >= i) {
                    break;
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        boolean isDecorView = lp.isDecor | isDecorView(child);
        lp.isDecor = isDecorView;
        if (!this.mInLayout) {
    ...
    private ItemInfo infoForCurrentScrollPosition() {
        ...
        while (i < this.mItems.size()) {
            ...
            if (!first && ii.position != lastPos + 1) {
                ...
                int i2 = lastPos + 1;
                ii.position = i2;
                ii.widthFactor = this.mAdapter.getPageWidth(i2);
                i--;
            }
    ...
    public class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public void onInitializeAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            if (event.getEventType() == 4096 && (pagerAdapter = ViewPager.this.mAdapter) != null) {
