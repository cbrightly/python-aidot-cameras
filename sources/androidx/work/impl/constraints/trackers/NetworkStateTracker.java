...
public class NetworkStateTracker extends ConstraintTracker<NetworkState> {
    ...
    public NetworkState getActiveNetworkState() {
        ...
        boolean isNotRoaming = true;
        boolean isConnected = info != null && info.isConnected();
        boolean isValidated = isActiveNetworkValidated();
        boolean isMetered = ConnectivityManagerCompat.isActiveNetworkMetered(this.mConnectivityManager);
    ...
    public boolean isActiveNetworkValidated() {
        ...
        try {
            NetworkCapabilities capabilities = this.mConnectivityManager.getNetworkCapabilities(this.mConnectivityManager.getActiveNetwork());
            if (capabilities == null || !capabilities.hasCapability(16)) {
                return false;
            }
            return true;
    ...
    public class NetworkStateCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onCapabilitiesChanged(@NonNull Network network, @NonNull NetworkCapabilities capabilities) {
            Logger.get().debug(NetworkStateTracker.TAG, String.format("Network capabilities changed: %s", new Object[]{capabilities}), new Throwable[0]);
