...
public class SettingItemAdapter extends AbsRecyclerAdapter<AbsViewBinder<SettingItem>, SettingItem> {
    ...
    /* access modifiers changed from: private */
    public boolean isMatched(@StringRes int desc) {
        for (int res : new int[]{R.string.dk_weak_network_switch, R.string.dk_item_block_switch, R.string.dk_crash_capture_switch, R.string.dk_cpu_detection_switch, R.string.dk_frameinfo_detection_switch, R.string.dk_ram_detection_switch}) {
            if (res == desc) {
                return true;
            }
        }
