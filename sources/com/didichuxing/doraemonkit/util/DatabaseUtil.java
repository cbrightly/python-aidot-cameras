package com.didichuxing.doraemonkit.util;

import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import java.lang.reflect.Array;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class DatabaseUtil {
    private DatabaseUtil() {
    }

    public static List<String> queryTableName(SQLiteDatabase database) {
        ArrayList<String> tableName = new ArrayList<>();
        if (database == null) {
            return tableName;
        }
        Cursor cursor = database.rawQuery("select name from sqlite_master where type='table' order by name", (String[]) null);
        while (cursor.moveToNext()) {
            tableName.add(cursor.getString(0));
        }
        cursor.close();
        return tableName;
    }

    public static String[] queryTableColumnName(SQLiteDatabase database, String tableName) {
        if (tableName == null || database == null) {
            return new String[0];
        }
        Cursor cursor = database.query(tableName, (String[]) null, (String) null, (String[]) null, (String) null, (String) null, (String) null);
        ArrayList<String> columnNames = new ArrayList<>(Arrays.asList(cursor.getColumnNames()));
        cursor.close();
        return (String[]) columnNames.toArray(new String[0]);
    }

    public static String[][] queryAll(SQLiteDatabase database, String tableName) {
        String[] strings = queryTableColumnName(database, tableName);
        Cursor cursor = database.query(tableName, (String[]) null, (String) null, (String[]) null, (String) null, (String) null, (String) null);
        int rowCount = cursor.getCount();
        int length = strings.length;
        int[] iArr = new int[2];
        iArr[1] = rowCount;
        iArr[0] = length;
        String[][] words = (String[][]) Array.newInstance(String.class, iArr);
        for (int y = 0; y < rowCount; y++) {
            if (cursor.moveToNext()) {
                for (int x = 0; x < strings.length; x++) {
                    if (cursor.getType(x) == 4) {
                        words[x][y] = new String(cursor.getBlob(x));
                    } else {
                        words[x][y] = cursor.getString(x);
                    }
                }
            }
        }
        cursor.close();
        return words;
    }
}
