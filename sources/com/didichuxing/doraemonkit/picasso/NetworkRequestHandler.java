...
public class NetworkRequestHandler extends RequestHandler {
    ...
    public RequestHandler.Result load(Request request, int networkPolicy) {
        ...
        DokitPicasso.LoadedFrom loadedFrom = response.cached ? DokitPicasso.LoadedFrom.DISK : DokitPicasso.LoadedFrom.NETWORK;
        ...
        if (loadedFrom == DokitPicasso.LoadedFrom.DISK && response.getContentLength() == 0) {
            Utils.closeQuietly(is);
            throw new ContentLengthException("Received response with 0 content-length header.");
        }
        if (loadedFrom == DokitPicasso.LoadedFrom.NETWORK && response.getContentLength() > 0) {
