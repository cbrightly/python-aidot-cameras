...
public abstract class AbstractDateDeserializer extends ContextObjectDeserializer implements ObjectDeserializer {
    ...
    public <T> T deserialze(DefaultJSONParser parser, Type clazz, Object fieldName, String format, int features) {
        ...
        if (lexer.token() == 2) {
            ...
            if (format == null) {
                val2 = null;
            } else if ("yyyy-MM-dd HH:mm:ss.SSSSSSSSS".equals(format) && (clazz instanceof Class) && ((Class) clazz).getName().equals("java.sql.Timestamp")) {
                return TypeUtils.castToTimestamp(strVal);
            } else {
                SimpleDateFormat simpleDateFormat = null;
                try {
                    simpleDateFormat = new SimpleDateFormat(format, parser.lexer.getLocale());
                } catch (IllegalArgumentException ex) {
                    if (format.contains(ExifInterface.GPS_DIRECTION_TRUE)) {
                        try {
                            simpleDateFormat = new SimpleDateFormat(format.replaceAll(ExifInterface.GPS_DIRECTION_TRUE, "'T'"), parser.lexer.getLocale());
            ...
            if (lexer.token() == 4) {
                if (JSON.DEFAULT_TYPE_KEY.equals(lexer.stringVal())) {
                    lexer.nextToken();
                    parser.accept(17);
                    Class<?> type = parser.getConfig().checkAutoType(lexer.stringVal(), (Class<?>) null, lexer.getFeatures());
                    if (type != null) {
                        clazz = type;
                    }
                    parser.accept(4);
