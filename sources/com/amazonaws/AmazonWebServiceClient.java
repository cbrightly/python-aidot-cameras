...
public abstract class AmazonWebServiceClient {
    ...
    public void setRegion(Region region2) {
        ...
        if (region2 != null) {
            ...
            if (region2.isServiceSupported(serviceName2)) {
                serviceEndpoint = region2.getServiceEndpoint(serviceName2);
                int protocolIdx = serviceEndpoint.indexOf("://");
                if (protocolIdx >= 0) {
                    serviceEndpoint = serviceEndpoint.substring("://".length() + protocolIdx);
                }
            } else {
                serviceEndpoint = String.format("%s.%s.%s", new Object[]{getEndpointPrefix(), region2.getName(), region2.getDomain()});
            }
            URI uri = toURI(serviceEndpoint);
            Signer signer2 = computeSignerByServiceRegion(serviceName2, region2.getName(), this.signerRegionOverride, false);
            synchronized (this) {
                this.endpoint = uri;
                this.signer = signer2;
            }
