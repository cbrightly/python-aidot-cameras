...
public class SoftWareCanvas extends Canvas {
    ...
    private Bitmap drawOnSFCanvas(Bitmap bitmap) {
        ...
        Bitmap sfBitmap = bitmap.copy(Bitmap.Config.ARGB_8888, false);
    ...
    private Paint replaceBitmapShader(Paint paint) {
        ...
        if (Build.VERSION.SDK_INT >= 26 && (paint.getShader() instanceof BitmapShader)) {
            ...
            try {
                Field mBitmap2 = BitmapShader.class.getField("mBitmap");
                mBitmap2.setAccessible(true);
                if (((Bitmap) mBitmap2.get(userBitmapShader)).getConfig() == Bitmap.Config.HARDWARE) {
                    ...
                    Bitmap sfBitmap = ((Bitmap) mBitmap2.get(userBitmapShader)).copy(Bitmap.Config.ARGB_8888, false);
                    this.bitmapWeakSet.add(sfBitmap);
                    Class cls = Integer.TYPE;
                    Constructor<BitmapShader> constructor = BitmapShader.class.getDeclaredConstructor(new Class[]{Bitmap.class, cls, cls});
                    constructor.setAccessible(true);
                    BitmapShader bitmapShaderShader = constructor.newInstance(new Object[]{sfBitmap, mTileX.get(userBitmapShader), mTileY.get(userBitmapShader)});
