...
public final class JsonTreeReader extends JsonReader {
    ...
    public String nextString() {
        ...
        if (token == jsonToken || token == JsonToken.NUMBER) {
        ...
        throw new IllegalStateException("Expected " + jsonToken + " but was " + token + locationString());
    }
    ...
    public double nextDouble() {
        ...
        if (token == jsonToken || token == JsonToken.STRING) {
    ...
    public void skipValue() {
        if (peek() == JsonToken.NAME) {
            ...
            int i = this.stackSize;
            if (i > 0) {
                this.pathNames[i - 1] = BuildConfig.TRAVIS;
            }
        }
