...
public final class MatrixUtil {
    ...
    static void buildMatrix(BitArray dataBits, ErrorCorrectionLevel ecLevel, Version version, int maskPattern, ByteMatrix matrix) {
        clearMatrix(matrix);
    ...
    static void embedTypeInfo(ErrorCorrectionLevel ecLevel, int maskPattern, ByteMatrix matrix) {
        ...
        for (int i = 0; i < typeInfoBits.getSize(); i++) {
            ...
            matrix.set(coordinates[0], coordinates[1], bit);
            if (i < 8) {
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(Version version, ByteMatrix matrix) {
        if (version.getVersionNumber() >= 2) {
            int[] coordinates = POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[version.getVersionNumber() - 1];
