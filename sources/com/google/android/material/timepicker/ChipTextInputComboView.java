...
public class ChipTextInputComboView extends FrameLayout implements Checkable {
    ...
    public boolean isChecked() {
        return this.f37chip.isChecked();
    }

    public void setChecked(boolean checked) {
        this.f37chip.setChecked(checked);
        int i = 0;
        this.editText.setVisibility(checked ? 0 : 4);
        Chip chip2 = this.f37chip;
        if (checked) {
            i = 8;
        }
    ...
    public void setText(CharSequence text) {
        this.f37chip.setText(formatText(text));
        if (!TextUtils.isEmpty(this.editText.getText())) {
    ...
    public void setOnClickListener(@Nullable View.OnClickListener l) {
        this.f37chip.setOnClickListener(l);
    }

    public void setTag(int key, Object tag) {
        this.f37chip.setTag(key, tag);
    }
    ...
    public void setChipDelegate(AccessibilityDelegateCompat clickActionDelegate) {
        ViewCompat.setAccessibilityDelegate(this.f37chip, clickActionDelegate);
    }
    ...
    public class TextFormatter extends TextWatcherAdapter {
        ...
        public void afterTextChanged(Editable editable) {
            if (TextUtils.isEmpty(editable)) {
                ChipTextInputComboView.this.f37chip.setText(ChipTextInputComboView.this.formatText(DEFAULT_TEXT));
            } else {
                ChipTextInputComboView.this.f37chip.setText(ChipTextInputComboView.this.formatText(editable));
            }
        }
    }
