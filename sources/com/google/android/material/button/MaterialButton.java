...
public class MaterialButton extends AppCompatButton implements Checkable, Shapeable {
    ...
    public void setIcon(@Nullable Drawable icon2) {
        if (this.icon != icon2) {
            this.icon = icon2;
            updateIcon(true);
            updateIconPosition(getMeasuredWidth(), getMeasuredHeight());
    ...
    public int[] onCreateDrawableState(int extraSpace) {
        ...
        if (isCheckable()) {
            Button.mergeDrawableStates(drawableState, CHECKABLE_STATE_SET);
        }
        if (isChecked()) {
            Button.mergeDrawableStates(drawableState, CHECKED_STATE_SET);
        }
