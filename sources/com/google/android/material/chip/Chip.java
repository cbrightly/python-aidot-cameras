...
public class Chip extends AppCompatCheckBox implements ChipDrawable.Delegate, Shapeable {
    ...
    public int[] onCreateDrawableState(int extraSpace) {
        ...
        if (isChecked()) {
            CheckBox.mergeDrawableStates(state, SELECTED_STATE);
        }
        if (isCheckable()) {
            CheckBox.mergeDrawableStates(state, CHECKABLE_STATE_SET);
        }
        return state;
    }
    ...
    private boolean handleAccessibilityExit(@NonNull MotionEvent event) {
        ...
        if (event.getAction() == 10) {
            try {
                Field f = cls.getDeclaredField("mHoveredVirtualViewId");
                f.setAccessible(true);
                if (((Integer) f.get(this.touchHelper)).intValue() != Integer.MIN_VALUE) {
                    Method m = cls.getDeclaredMethod("updateHoveredVirtualView", new Class[]{Integer.TYPE});
    ...
    public class ChipTouchHelper extends ExploreByTouchHelper {
        ...
        /* access modifiers changed from: protected */
        public void getVisibleVirtualViews(@NonNull List<Integer> virtualViewIds) {
        ...
        /* access modifiers changed from: protected */
        public void onPopulateNodeForVirtualView(int virtualViewId, @NonNull AccessibilityNodeInfoCompat node) {
