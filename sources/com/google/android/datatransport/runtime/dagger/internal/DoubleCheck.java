...
public final class DoubleCheck<T> implements a<T>, Lazy<T> {
    ...
    private DoubleCheck(a<T> provider2) {
        if (provider2 != null) {
    ...
    public static <P extends a<T>, T> a<T> provider(P delegate) {
    ...
    public static <P extends a<T>, T> Lazy<T> lazy(P provider2) {
        if (provider2 instanceof Lazy) {
            return (Lazy) provider2;
        }
        return new DoubleCheck((a) Preconditions.checkNotNull(provider2));
    }
}
