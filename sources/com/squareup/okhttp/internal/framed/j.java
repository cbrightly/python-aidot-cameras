...
public class j {
    ...
    public byte[] c(byte[] buf) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        a node = this.d;
        int current = 0;
        int nbits = 0;
        for (byte b2 : buf) {
            current = (current << 8) | (b2 & 255);
            nbits += 8;
            while (nbits >= 8) {
                node = node.a[(current >>> (nbits - 8)) & 255];
                if (node.a == null) {
                    baos.write(node.b);
                    nbits -= node.c;
                    node = this.d;
        ...
        while (nbits > 0) {
            a node2 = node.a[(current << (8 - nbits)) & 255];
            if (node2.a != null || node2.c > nbits) {
                break;
            }
            baos.write(node2.b);
            nbits -= node2.c;
    ...
    private void a(int sym, int code, byte len) {
        a terminal = new a(sym, len);
        a current = this.d;
        while (len > 8) {
            len = (byte) (len - 8);
            int i = (code >>> len) & 255;
