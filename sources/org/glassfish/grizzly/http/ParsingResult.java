...
public final class ParsingResult implements Cacheable {
    ...
    public static ParsingResult create(HttpContent httpContent2, Buffer remainderBuffer2) {
        ParsingResult resultObject = (ParsingResult) ThreadCache.takeFromCache(CACHE_IDX);
        if (resultObject == null) {
            resultObject = new ParsingResult();
        }
        resultObject.httpContent = httpContent2;
        resultObject.remainderBuffer = remainderBuffer2;
        return resultObject;
    }

    public static ParsingResult create(HttpContent httpContent2, Buffer remainderBuffer2, boolean sendHeaderUpstream2) {
        ParsingResult resultObject = create(httpContent2, remainderBuffer2);
        resultObject.sendHeaderUpstream = sendHeaderUpstream2;
        return resultObject;
    }
