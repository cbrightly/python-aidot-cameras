...
public class EchoFilter extends BaseFilter {
    ...
    public NextAction handleRead(FilterChainContext ctx) {
        ...
        if (logger2.isLoggable(level)) {
            logger2.log(level, "EchoFilter. connection={0} dstAddress={1} message={2}", new Object[]{connection, address, message});
        }
        if (message instanceof Buffer) {
            ((Buffer) message).allowBufferDispose(true);
        }
        ctx.write(address, message, (CompletionHandler<WriteResult>) null);
