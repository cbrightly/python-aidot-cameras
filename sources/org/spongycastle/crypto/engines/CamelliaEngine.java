...
public class CamelliaEngine implements BlockCipher {
    ...
    private void e(int[] s, int[] skey, int keyoff) {
        ...
        int u = iArr[t1 & 255];
        int[] iArr2 = e;
        int u2 = u ^ iArr2[(t1 >>> 8) & 255];
        int[] iArr3 = d;
        int u3 = u2 ^ iArr3[(t1 >>> 16) & 255];
        int[] iArr4 = b;
        int u4 = u3 ^ iArr4[(t1 >>> 24) & 255];
        int t2 = s[1] ^ skey[keyoff + 1];
        int v = ((iArr4[t2 & 255] ^ iArr[(t2 >>> 8) & 255]) ^ iArr2[(t2 >>> 16) & 255]) ^ iArr3[(t2 >>> 24) & 255];
        s[2] = s[2] ^ (u4 ^ v);
        s[3] = s[3] ^ ((u4 ^ v) ^ n(u4, 8));
        int t12 = s[2] ^ skey[keyoff + 2];
        int u5 = ((iArr[t12 & 255] ^ iArr2[(t12 >>> 8) & 255]) ^ iArr3[(t12 >>> 16) & 255]) ^ iArr4[(t12 >>> 24) & 255];
        int t22 = s[3] ^ skey[keyoff + 3];
        int v2 = ((iArr[(t22 >>> 8) & 255] ^ iArr4[t22 & 255]) ^ iArr2[(t22 >>> 16) & 255]) ^ iArr3[(t22 >>> 24) & 255];
        s[0] = s[0] ^ (u5 ^ v2);
        s[1] = s[1] ^ ((u5 ^ v2) ^ n(u5, 8));
    }

    private void g(int[] s, int[] fkey, int keyoff) {
        s[1] = s[1] ^ k(s[0] & fkey[keyoff + 0], 1);
        s[0] = s[0] ^ (fkey[keyoff + 1] | s[1]);
        s[2] = s[2] ^ (fkey[keyoff + 3] | s[3]);
        s[3] = k(s[2] & fkey[keyoff + 2], 1) ^ s[3];
    }

    private void q(boolean forEncryption, byte[] key) {
        byte[] bArr = key;
        int[] k2 = new int[8];
        int[] ka = new int[4];
        int[] kb = new int[4];
    ...
    private int l(byte[] in, int inOff, byte[] out, int outOff) {
        for (int i2 = 0; i2 < 4; i2++) {
            this.k[i2] = d(in, (i2 * 4) + inOff);
            int[] iArr = this.k;
            iArr[i2] = iArr[i2] ^ this.i[i2];
        }
        e(this.k, this.h, 0);
        e(this.k, this.h, 4);
        e(this.k, this.h, 8);
        ...
        j(this.k[3], out, outOff + 4);
        j(this.k[0], out, outOff + 8);
        j(this.k[1], out, outOff + 12);
        return 16;
    }
    ...
    public void a(boolean forEncryption, CipherParameters params) {
        if (params instanceof KeyParameter) {
            q(forEncryption, ((KeyParameter) params).a());
            this.f = true;
            return;
        }
