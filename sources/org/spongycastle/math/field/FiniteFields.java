package org.spongycastle.math.field;

import java.math.BigInteger;

public abstract class FiniteFields {
    static final FiniteField a = new PrimeField(BigInteger.valueOf(2));
    static final FiniteField b = new PrimeField(BigInteger.valueOf(3));

    public static PolynomialExtensionField a(int[] exponents) {
        if (exponents[0] == 0) {
            int i = 1;
            while (i < exponents.length) {
                if (exponents[i] > exponents[i - 1]) {
                    i++;
                } else {
                    throw new IllegalArgumentException("Polynomial exponents must be montonically increasing");
                }
            }
            return new GenericPolynomialExtensionField(a, new GF2Polynomial(exponents));
        }
        throw new IllegalArgumentException("Irreducible polynomials in GF(2) must have constant term");
    }

    public static FiniteField b(BigInteger characteristic) {
        int bitLength = characteristic.bitLength();
        if (characteristic.signum() <= 0 || bitLength < 2) {
            throw new IllegalArgumentException("'characteristic' must be >= 2");
        }
        if (bitLength < 3) {
            switch (characteristic.intValue()) {
                case 2:
                    return a;
                case 3:
                    return b;
            }
        }
        return new PrimeField(characteristic);
    }
}
