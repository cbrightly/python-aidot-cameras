...
| `AIDOT_FAST_CONNECT_HOST_ONLY` | Within `AIDOT_FAST_CONNECT`, narrows only the LOCAL `RTCPeerConnection` to host candidates (skips the ~5 s srflx gather stall where outbound STUN is unanswered). The camera-facing `IceServerList` keeps STUN, so the camera still gathers. **On-subnet only** — drops srflx/relay fallback, so off-subnet / strict-NAT viewers must leave it off. Opt-in. | unset (off) |
