...
| `AIDOT_OFFER_PCMU` | Put PCMU back in the audio offer. It is dropped by default: every camera measured sends audio as PCMA (payload type 8), while payload type 0 - PCMU's - is what one A000088 announces and uses for VIDEO, and a payload type cannot belong to two receivers. `1` restores it. | `0` (dropped) |
