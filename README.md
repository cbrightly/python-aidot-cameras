...
| `AIDOT_SKIP_DOOMED_SERVE` | Whether an open that observed **no video at all** abandons to the retry instead of launching a serve there is nothing to feed. Measured 2026-09-05 on a camera that dropped off the WiFi: every attempt ran the full first-media wait, served anyway, and ffmpeg died at once on `Could not find codec parameters ... unspecified size` / `dimensions not set` / `Could not write header` - 11 attempts and 6 stalls in 25 minutes, each spawning an ffmpeg that could not start. Only applies to opens that actually build a serve: press-to-talk, the siren and snapshots share this path, need no inbound video, and are never abandoned. Set to `0`/`false`/`no`/`off` to go back to launching the serve on the timeout path; a malformed value is ignored rather than allowed to alter a media path. | enabled (on) |
...
| `AIDOT_PUBLISH_GAP_WARN_S` | Seconds without a frame to publish before the direct publisher says so in the log, with the queue depth at the time. Each session also records its largest gap. `0` disables the line. | `1.0` |
