...
class TutkStreamSession:
    ...
    def _start_sync(self) -> bool:
        ...
        if ret < 0:
            _LOGGER.debug(
                "IOTC_Initialize2 returned %d (may already be initialized)", ret
            )
        else:
        ...
        av.avSendIOCtrl(
            self._av_index, self._IOTYPE_INNER_SND_DATA_DELAY, (ctypes.c_uint8 * 2)(), 2
        )
        # 2. IOTYPE_USER_IPCAM_START (511) - 8-byte body (all zeros = default stream)
        av.avSendIOCtrl(
            self._av_index, self._IOTYPE_USER_IPCAM_START, (ctypes.c_uint8 * 8)(), 8
        )
        # 3. IOTYPE_USER_IPCAM_AUDIOSTART (768) - 8-byte body
        av.avSendIOCtrl(
            self._av_index,
            self._IOTYPE_USER_IPCAM_AUDIOSTART,
            (ctypes.c_uint8 * 8)(),
            8,
        )
