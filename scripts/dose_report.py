            ...
            out.append(
                {
                    "arm": m.group(1),
                    "cycle": int(m.group(2)),
                    "t0": datetime.datetime.strptime(m.group(3), "%Y-%m-%d %H:%M:%S"),
                }
            )
        m = re.match(r"BLOCK (\w) cycle(\d) end   (\S+ \S+)", ln)
        if m and out:
            out[-1]["t1"] = datetime.datetime.strptime(m.group(3), "%Y-%m-%d %H:%M:%S")
...
def latencies(log, t0, t1):
    ...
    d = sorted(
        round((rx[p] - t).total_seconds(), 2)
        for p, t in sent.items()
        if p in rx and rx[p] >= t
    )
    return len(sent), d
...
def main(halog, runlog):
    res = {}
    print(
        f"{'block':10s} {'opens':>6s} {'answered':>9s} {'median':>8s} {'p90':>8s} {'max':>8s}"
    )
    ...
    if {"A1", "B1", "A2", "B2"} <= res.keys():
        ...
        c2 = res["B2"] - res["A2"]
        print(
            f"dose effect (B-A):  cycle1 {c1:+.2f}s   cycle2 {c2:+.2f}s"
            f"   -> {'consistent' if c1 * c2 > 0 else 'INCONSISTENT'}"
        )
        print(
            f"time trend (c2-c1): armA {res['A2'] - res['A1']:+.2f}s"
            f"   armB {res['B2'] - res['B1']:+.2f}s"
        )
        print()
        if c1 > 0.5 and c2 > 0.5:
            print(
                "READ: latency tracks OUR open rate on both cycles"
                " - the churn is at least partly self-sustaining."
            )
        elif max(res.values()) < 2.0:
            print(
                "READ: no degradation at any dose. Open rate is not the driver"
