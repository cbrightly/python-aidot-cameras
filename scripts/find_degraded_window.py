...
def main(path):
    ...
    for ln in lines:
        ...
        if "webrtc rx" in ln and "webrtcResp" in ln:
            ...
            continue
        for key, needle in (
            ("open_fail", "DTLS serve: open failed"),
    ...
    if not per_hour:
        print("no serve-loop opens in this log")
        return
    print(
        f"{'hour':16s} {'opens':>6s} {'ans':>5s} {'med':>7s} {'p90':>7s} "
    ...
    for h in sorted(per_hour):
        ...
        if not d:
            print(
                f"{h:%Y-%m-%d %H:00} {c['opens']:6d} {0:5d} "
        ...
        print(
            f"{h:%Y-%m-%d %H:00} {c['opens']:6d} {len(d):5d} {med:7.2f} "
    ...
    for h, med, n, c in degraded:
        print(
            f"  {h:%Y-%m-%d %H:00}  median {med:.2f}s over {n} answers; "
